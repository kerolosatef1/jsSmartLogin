...
function signUP(){
    if(signUpName.value=='' && signUpEmail.value=='' && signUpPassword.value==''){
        emailExit.innerHTML=`<span class="alert alert-danger">All fields are required</span>`;
       
    }
   else if(signUpName.value=!null && signUpEmail.value=='' && signUpPassword.value==''){
        emailExit.innerHTML=`<span class="alert alert-danger">Please Enter Email and Password</span>`;
       
    }
    else if(signUpName.value!=null && signUpEmail.value!=null && signUpPassword.value==''){
        emailExit.innerHTML=`<span class="alert alert-danger">Please Enter  Password</span>`;
       
    }

    else if(signUpName.value!=null && signUpEmail.value=="" && signUpPassword.value!=null){
        emailExit.innerHTML=`<span class="alert alert-danger">Please Enter  Email</span>`;
       
    }
    else if(signUpName.value=="" && signUpEmail.value!=null&& signUpPassword.value!=null){
        emailExit.innerHTML=`<span class="alert alert-danger">Please Enter  Name</span>`;
       
    }
    else if(signUpName.value=="" && signUpEmail.value=="" && signUpPassword.value!=null){
        emailExit.innerHTML=`<span class="alert alert-danger">Please Enter  Name and Email</span>`;
        ...
        for(let i=0;i<users.length;i++){
            if(users[i].email.toLowerCase()==signUpEmail.value.toLowerCase()){
                emailExit.innerHTML=`<span class="alert alert-danger">Email already exists</span>`;
                return false;
            }
        }
        addUser();
